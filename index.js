...
server.post('/IRCTC-find-trains', (req, res) => {
    ...
    request('https://api.railwayapi.com/v2/name-to-code/station/'+source+'/apikey/'+apiKey+'/', function(error, responseFromAPI, body) {
  		if (error) {
  			...
  			var srcBody = JSON.parse(body);
        if(srcBody && srcBody.stations && srcBody.stations.length >= 1) {
          var src = srcBody.stations[0].code;
          srcStationCode = src;
          request('https://api.railwayapi.com/v2/name-to-code/station/'+destination+'/apikey/'+apiKey+'/', function(error, responseFromAPI, body) {
        		if (error) {
        			console.log("ERR:", error);
        		} else {
        			var destBody = JSON.parse(body);
              if(destBody && destBody.stations && destBody.stations.length >= 1) {
                var dest = destBody.stations[0].code;
                destStationCode = dest;
                if(srcStationCode && destStationCode && date) {
                  ...
                  request(reqUrl, function(error, responseFromAPI, body) {
                      ...
                      if(trains && trains.length >= 1) {
                        var trainsList = [];
                          ...
                          var trainInfo = replacedStr.toLowerCase() + ' at ' + trains[i].src_departure_time;
                ...
                return res.json({
                    speech: 'Invalid destination name',
                    displayText: 'Invalid destination name',
                    source: 'IRCTC-find-trains'
                });
              }
        		}
          ...
          return res.json({
              speech: 'Invalid source name',
              displayText: 'Invalid source name',
              source: 'IRCTC-find-trains'
          });
        }
  		}
